  ...
  return db
    ...
    .then(() => {
      ...
      const insertArticlesQueryStr = format(
        "INSERT INTO articles (title, topic, body, created_at, votes, article_img_url) VALUES %L RETURNING *;",
        formattedArticleData.map(
          ({ title, topic, body, created_at, votes = 0, article_img_url }) => [
            title,
            topic,
            body,
            created_at,
    ...
    .then(({ rows: articleRows }) => {
      ...
      if (commentData.length > 0) return db.query(insertCommentsQueryStr);
    });
};
